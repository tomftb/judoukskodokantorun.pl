<?php
if(!defined('TREN42')) { exit('NO PERMISSION');}
echo '<div class="DIV_MAIN">';
$ID_TREN=$_GET["ID"];
$status_popraw=0;
$err_obraz_tab=array();
echo '<a class="A_BACK"href="'.PAGE_URL.'&IDW=0&IDS='.$IDS.'$ID'.$ID.'"><p class="P_HREF_BACK">Powrót</p></a>';
echo '<p class="P_MAIN">Edycja Trenera o nr ID - [<span style="color:purple;">'.$ID.'</span>]</p>';
...
//---------------------------------------------------------------IMG----------------------------------------------------------------------------------
$SEL_IMG = $db->query("select ID,NR_IMG,NAZWA_I,WIDTH,HEIGHT,NAZWA_I_M,M_WIDTH,M_HEIGHT FROM TREN_IMG where ID_TREN='".$ID."' AND WSK_U=0");
for ($z=1; $z<2; $z++){
$IST_IMG = $db->query("select ID FROM TREN_IMG where ID_TREN='$ID_TREN' AND NR_IMG='$z'");
$IMG_POM = mysqli_num_rows($IST_IMG);
					if ($IMG_POM!=0){
									$SEL_IMG2 = $db->query("select ID,NR_IMG,NAZWA_I,WIDTH,HEIGHT,NAZWA_I_M,M_WIDTH,M_HEIGHT FROM TREN_IMG where ID_TREN='".$ID."' AND NR_IMG='$z' AND WSK_U=0");
									...
									echo "<a HREF=\"javascript:displayWindow('".APP_URL."/zdjecia/trener/$rek_img[2]',$rek_img[0],$rek_img[3],$rek_img[4])\">";
									echo '<img src="'.APP_URL.'/zdjecia/trener/'.$rek_img[5].'" alt="Zdjecie '.$rek_img[1].'" style="WIDTH:'.$rek_img[6].'px; HEIGHT:'.$rek_img[7].'px; border:0px; margin:10px;" />';
...
//---------------------------------------------------------------KONIEC-IMG----------------------------------------------------------------------------------
$SEL_TREN=$db->query("SELECT ID,ID_COL_IMIE_N,ID_COL_INFO,IMIE_N,INFO,R_IMIE_N,R_INFO,ID_P_IMI_N,ID_P_INFO,CSS_IMI_N,CSS_INFO FROM TREN WHERE ID=".$ID) or die ("<p class=\"P_SQL_ERR\">Błąd zapytania SQL - <span class=\"S_SQL\">SEL_TREN : ".mysqli_error()."</span></p>");
...
echo '</div>';
echo '<input type="hidden" name="ID_TREN" value="'.$ID.'" />';
...
//--------------------------------------------------------------------KONIEC-UPDATE-ZDJECIE---------------------------------------------------------------------------------------		
																		...
																		echo '<a class="A_BACK" href="'.PAGE_URL.'&IDW=0&IDS='.$IDS.'#ID'.$_POST['ID_TREN'].'"><p class="P_HREF_BACK2">Powrót do MENU - Trenerzy</p></a>';
